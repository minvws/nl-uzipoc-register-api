...
from app.router import ExchangeRequest
...
class Service:
    ...
    def _get_request_claims(self, request: Request) -> Dict[str, Any]:
        ...
        try:
            scheme, raw_jwt = get_authorization_scheme_param(request.headers.get("Authorization"))
            ...
            return json.loads(request_jwt.claims) if isinstance(request_jwt.claims, str) else request_jwt.claims
        except InvalidJWSObject:
            logger.warning(f"Invalid jwt received: {request.headers.get('Authorization')}")
            raise UnauthorizedError(f"Invalid jwt received")

    def _fetch_irma_result(self, exchange_token: str) -> Any:
        irma_response = requests.get(
            f"{self._irma_controller_result_url}/{exchange_token}",
        )
        if irma_response.status_code >= 400:
            raise UnauthorizedError(f"Received invalid response({irma_response.status_code}) from IRMA")
    ...
    def handle_request(
        ...
        jwt_payload["relations"] = [r for r in jwt_payload["relations"] if r["ura"] == irma_response_json["uziId"]]
        jwt_payload["req_iss"] = self._issuer
        jwt_payload["x5c"] = claims["x5c"]
        jwt_payload["loa_authn"] = claims["loa_authn"]
        jwe_token = create_jwe(
            self._jwt_sign_priv_key,
            self._jwt_sign_crt_path,
            jwe_pub_key,
            jwt_payload
        )
        headers = {
